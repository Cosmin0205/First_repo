...
for i in range(1, len(masini) - 1):
    masini[i] = masini[i].upper()  # pozitia din lista va fi scrisa cu majuscula
    print(masini)
...
for marca in masini:
    if marca == 'Trabant' or marca == 'Lastun':
        continue  # daca gaseste marcile de mai sus ,sare peste
...
for marca in masini:
    if marca == 'Trabant' or marca == 'Lastun':
        masini_vechi.append(marca)  # se adauga in noua lista
        masini.remove(marca)  # se elimina din lista veche
        masini.append('Tesla')  # se adauga noul element
...
buget = 15000
for masina, pret in pret_masini.items():  # dict masina,pret
    if pret <= buget:
...
for i in numere:
    if i > 0:
        i = i - i * 2  # valoare negativa a unui nr
    negativ.append(i)  # se inlocuieste
print(negativ)

#______________________________-exercitii optionale
# # 1.
